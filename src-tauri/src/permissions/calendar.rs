...
mod native {
    ...
    pub fn check() -> Result<Permission, PermissionError> {
        let status = unsafe {
            status_from_raw(EKEventStore::authorizationStatusForEntityType(EKEntityType::Event).0)
        };
        Ok(Permission::new("calendar", "Calendar", "Access to read and modify your calendars", status, false))
    }
...
pub fn check() -> Result<Permission, PermissionError> {
    super::non_macos_permission("calendar", "Calendar", "Access to read and modify your calendars", false)
}
#[cfg(not(target_os = "macos"))]
