...
async fn handle_read(params: &Value, id: Value) -> JsonRpcResponse {
    ...
    let metadata = match fs::metadata(path) {
        Ok(m) => m,
        Err(e) => return generic_error(id, format!("Failed to read file metadata: {}", e)),
    };
    ...
    match encoding {
        "base64" => {
            ...
            let mut file = match fs::File::open(path) {
                Ok(f) => f,
                Err(e) => return generic_error(id, format!("Failed to open file: {}", e)),
            };
        ...
        _ => {
            ...
            match fs::read_to_string(path) {
                ...
                Err(e) => generic_error(id, format!("Failed to read as UTF-8: {}. Try encoding='base64'.", e)),
            }
        }
    }
}
...
async fn handle_write(params: &Value, id: Value) -> JsonRpcResponse {
    ...
    let bytes = match encoding {
        "base64" => match BASE64.decode(content) {
            Ok(b) => b,
            Err(e) => return generic_error(id, format!("Invalid base64 content: {}", e)),
        },
    ...
    let result = if append {
        ...
        let mut file = match fs::OpenOptions::new().append(true).create(true).open(path) {
            Ok(f) => f,
            Err(e) => return generic_error(id, format!("Failed to open file for appending: {}", e)),
        };
